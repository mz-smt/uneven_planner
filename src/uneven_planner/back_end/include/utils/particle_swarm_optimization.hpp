//
// Created by mazheng on 25-4-29.
//

#ifndef SRC_PARTICLE_SWARM_OPTIMIZATION_HPP
#define SRC_PARTICLE_SWARM_OPTIMIZATION_HPP
...
#define ParallelPolicy THREAD_POOL
#define OPENMP_PARALLEL 0  // most efficient (cpu time:1)
#define THREAD_POOL 1  // mid efficient (cpu time:1.5)
#define MUL_THREADS 2  // least efficient (cpu time:5)

#if ParallelPolicy == THREAD_POOL
#include <memory>

#include "thread_pool.hpp"
#elif ParallelPolicy == MUL_THREADS
#include <thread>
#endif

/**
 * @brief Particle swarm optimization algorithm
 * @attention This header file is reference to https://github.com/Rookfighter/pso-cpp
 * */

namespace uneven_planner::pso {
    ...
    public:
        ConstantWeight() : ConstantWeight(1.0) {}
        ...
        explicit ConstantWeight(const Scalar weight) : weight_(weight) {}

        Scalar operator()(const Index, const Index) const {
            return weight_;
        }
    };
    ...
    public:
        LinearDecrease() : LinearDecrease(0.4, 0.9) {}
        ...
        LinearDecrease(const Scalar minval, const Scalar maxval)
                : weightMin_(minval), weightMax_(maxval) {}

        Scalar operator()(const Index iteration, const Index maxIt) const {
    ...
    public:
        ExponentialDecrease1() : ExponentialDecrease1(0.4, 0.9) {}
    ...
    public:
        ExponentialDecrease2() : ExponentialDecrease2(0.4, 0.9) {}
    ...
    public:
        ExponentialDecrease3() : ExponentialDecrease3(0.4, 0.95, 0.2, 7.0) {}
        ...
        ExponentialDecrease3(const Scalar minval, const Scalar maxval, const Scalar d1,
                             const Scalar d2)
    ...
    private:
        ...
        void evaluateObjective(const Matrix& particles, Vector& fvals) {
#if ParallelPolicy == OPENMP_PARALLEL
            #pragma omp parallel for num_threads(threads_)
            for (Index i = 0; i < particles.cols(); ++i)
                fvals(i) = objective_(particles.col(i));
#elif ParallelPolicy == THREAD_POOL
            std::vector<std::future<void>> results;
            Index start_index = 0;
            for (auto thread_num = 0; thread_num < threads_; ++thread_num) {
                Index end_index = (thread_num + 1) * particles.cols() / threads_;
        ...
        void calculateVelocities(const Matrix& particles, const Matrix& bestParticles,
                                 const Index gbest, const Index iteration, Matrix& velocities) {
            ...
            for (Index i = 0; i < velocities.cols(); ++i) {
                for (Index j = 0; j < velocities.rows(); ++j) {
                    ...
                    if (maxVel_ > 0) vel = std::min(maxVel_, std::max(-maxVel_, vel));
        ...
        Result _minimize(const Matrix& bounds, Matrix& particles) {
            ...
            while ((maxIt_ == 0 || iterations < maxIt_) && fchange > feps_ && xchange > xeps_) {
                ...
                bool callbackResult = callback_(iterations, bestParticles, bestFvals, gbest);

                if (verbosity_ > 0) {
                    std::stringstream ss;
                    ss << "it=" << std::setfill('0') << std::setw(4) << iterations << std::fixed
                       << std::showpoint << std::setprecision(6) << "    fchange=" << fchange
                       << "    xchange=" << xchange;
                    ...
                    if (verbosity_ > 1) ss << "    xval=" << vector2str(bestParticles.col(gbest));
    ...
    public:
        ParticleSwarmOptimization()
                : objective_(),
                  ...
                  phip_(static_cast<Scalar>(2.0)),
                  phig_(static_cast<Scalar>(2.0)),
                  maxVel_(static_cast<Scalar>(0.0)),
                  verbosity_(0),
                  dice_() {
        ...
        Result minimize(const Matrix& bounds, const Index cnt) {
#if ParallelPolicy == THREAD_POOL
            thread_pool_ = std::make_unique<ThreadPool>(threads_);
#endif
            if (cnt == 0) throw std::runtime_error("particle count cannot be 0");
            if (bounds.rows() != 2)
        ...
        Result minimize(const Matrix& bounds, const Index cnt, const Vector& initGuess) {
            if (cnt == 0) throw std::runtime_error("particle count cannot be 0");
            if (bounds.rows() != 2)
                throw std::runtime_error("bounds has not exactly 2 rows (min, max)");
            for (Index i = 0; i < bounds.cols(); ++i) {
        ...
        Result minimize(const Matrix& bounds, Matrix& particles) {
            if (bounds.rows() != 2)
                throw std::runtime_error("bounds has not exactly 2 rows (min, max)");
        ...
        void getRandomParticles(const Matrix& bounds, const Index cnt, Matrix& particles) {
...
#endif //SRC_PARTICLE_SWARM_OPTIMIZATION_HPP
