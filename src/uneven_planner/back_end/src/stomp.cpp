                              ...
                              Eigen::MatrixXd& trajectory_joints)
{
    ...
    {
        std::cout << "Control Cost Matrix is not square" << std::endl;
        return false;
    }
...
namespace uneven_planner
{
    ...
    bool Stomp::setPlanTask(std::vector<Eigen::Vector3f> &path) {
        for (auto point : path) {
            std::cout << "debug compute cost path: " << point.x() << " " << point.y() << " " << point.z() << std::endl;
        }
        config_.num_timesteps = path.size();
        auto parameters = pathToMatrix(path);
        Eigen::MatrixXd parameter_output;
        bool result = solve(parameters, parameter_output);
        if (result) {
            path = matrixToPath(parameter_output);
        }
        return result;
    }
                      ...
                      Eigen::MatrixXd& parameters_optimized)
    {
        ...
        {
            std::cout << "Unable to generate initial trajectory" << std::endl;
        }
    ...
    bool Stomp::solve(const Eigen::MatrixXd& initial_parameters, Eigen::MatrixXd& parameters_optimized)
    {
        ...
        std::cout << "set init solution" << std::endl;
        ...
        {
            std::cout << "Initial trajectory dimensions is incorrect" << std::endl;
            return false;
        }
        else
        {
            ...
            {
                std::cout << "Initial trajectory number of time steps is incorrect" << std::endl;
                return false;
            }
        }
        ...
        std::cout << "debug optimize before" << std::endl;
        // computing initialial trajectory cost
        if (!computeOptimizedCost())
        {
            std::cout << "Failed to calculate initial trajectory cost" << std::endl;
            return false;
        }
        std::cout << "debug optimize after: " << parameters_total_cost_ << std::endl;
        parameters_valid_prev_ = parameters_valid_;
        while (current_iteration_ <= config_.num_iterations && runSingleIteration())
        {
            std::cout << "STOMP completed iteration:" << current_iteration_ << "with cost: " << current_lowest_cost_ << std::endl;
            if (parameters_valid_)
            {
                std::cout << "Found valid solution, will iterate " << config_.num_iterations_after_valid - valid_iterations << std::endl;
                valid_iterations++;
            }
            else
            {
        ...
        if (parameters_valid_)
        {
            std::cout << "STOMP found a valid solution with cost: " << current_lowest_cost_ << " after: " << current_iteration_ << std::endl;
        }
        else
        {
            if (proceed_)
                std::cout << "STOMP failed to find a valid solution after iterations: " << current_iteration_ << std::endl;
            else
                std::cout << "Stomp was terminated" << std::endl;
        }
    ...
    bool Stomp::resetVariables()
    {
        ...
        {
            std::cout << "'max_rollouts' must be greater than 'num_rollouts_per_iteration'." << std::endl;
    ...
    bool Stomp::cancel()
    {
        std::cout << "Interrupting STOMP" << std::endl;
        proceed_ = false;
        return !proceed_;
    }
    ...
    bool Stomp::generateNoisyRollouts()
    {
        ...
        for (auto r = 0u; r < rollouts_generate; r++)
        {
            ...
            {
                std::cout << "Failed to generate noisy parameters at iteration " << current_iteration_ << std::endl;
                return false;
            }
        }
    ...
    bool Stomp::filterNoisyRollouts()
    {
        ...
        for (auto r = 0u; r < config_.num_rollouts; r++)
        {
            ...
            {
                std::cout << "Failed to filter noisy parameters" << std::endl;
                return false;
            }
    ...
    bool Stomp::computeRolloutsStateCosts()
    {
        ...
        for (auto r = 0u; r < config_.num_rollouts; r++)
        {
            ...
            {
                std::cout << "Trajectory cost computation failed for rollout" << r << std::endl;
                proceed = false;
                break;
            }
        }
    ...
    bool Stomp::updateParameters()
    {
        ...
        {
            std::cout << "Updates filtering step failed" << std::endl;
            return false;
        }
