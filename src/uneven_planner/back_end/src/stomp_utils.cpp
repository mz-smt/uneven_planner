...
namespace uneven_planner {
    ...
    Eigen::MatrixXd pathToMatrix(const std::vector<Eigen::Vector3f>& path) {
        std::size_t num_timesteps = path.size();
        Eigen::MatrixXd parameters(3, num_timesteps);  // 只生成 3 行：x,y,theta

        for (std::size_t t = 0; t < num_timesteps; ++t) {
            parameters(0, t) = path[t].x();
            parameters(1, t) = path[t].y();
            parameters(2, t) = path[t].z();
        }
        return parameters;
    }

    std::vector<Eigen::Vector3f> matrixToPath(const Eigen::MatrixXd& parameters) {
        std::size_t num_timesteps = parameters.cols();        // 列数即路径点数:contentReference[oaicite:1]{index=1}
        std::vector<Eigen::Vector3f> path;
        path.reserve(num_timesteps);

        for (std::size_t t = 0; t < num_timesteps; ++t) {
            // 提取第 t 列的前三个元素：x,y,theta
            Eigen::Vector3f pose;
            pose.x() = static_cast<float>(parameters(0, t));
            pose.y() = static_cast<float>(parameters(1, t));
            pose.z() = static_cast<float>(parameters(2, t));
            path.push_back(pose);
        }
        return path;
    }
